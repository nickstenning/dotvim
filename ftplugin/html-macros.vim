...
noremap <buffer> <C-K> :!$BROWSER file:$PWD/% &<CR><Esc>
...
noremap <buffer> <F1> :if strpart(getline(1),2,7) !~ "^DOCTYPE"<CR>0read ~/.vim/html_template.html<CR>normal /<\/title\\|<\/TITLE<CR><CR>:endif<CR>3jf>a
...
nnoremenu 40.401 Html.template\ \ \ \ \ \ \ \ F1 :if strpart(getline(1),0,6) !~ "^<\[Hh]\[Tt]\[Mm]\[Ll]>"<CR>0read ~/.vim/html_template.html<CR>normal /<\/title\\|<\/TITLE<CR><CR>:endif<CR>i
